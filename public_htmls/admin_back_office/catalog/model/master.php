<?php 
    class MasterModel extends db {
        ...
        public function getSumPayment_by_type_payment($date='', $table_arr_id = ''){
            $result = array();
            $where = ' WHERE id<>0 ';
            if(!empty($date)){
                $where .= " AND res_payment.date_create like '".$date." %' ";
            }
            if ($table_arr_id) {
                $where .= " AND res_payment.table_id IN (" . $table_arr_id . ')';
            }
            $result = $this->query("SELECT DATE(date_create) AS order_date, type_payment, SUM(amount) AS total_amount, id FROM res_payment ".$where." GROUP BY type_payment");
            return $result;
        }
