<?php
    class MasterModel extends db {
        ...
        public function getMenuByOrderId($id){
            $result = array();
            $result_menu = $this->query("SELECT * FROM res_order INNER JOIN res_menu ON res_order.menu_id = res_menu.id WHERE res_order.id = ".(int)$id." LIMIT 0,1");
            $result = $result_menu->row;
            return $result;
        }
        ...
        {
            $token = "GZowpwR1b7IXK59uk3nLeCv8cbbJDx3XijjII396BuK"; // ใส่ Token ที่สร้างไว้
