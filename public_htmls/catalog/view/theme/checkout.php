...
<!--                    </li>-->
                    <li class="nav-item">
                        <a class="nav-link" id="qrImageTab" data-toggle="tab" href="#qrImage" role="tab" aria-controls="qrImage" aria-selected="false">สแกนได้เลยจร้า</a>
                    </li>
                </ul>
...
<script>
    $(document).on('click','#QRcode',function(e){
        ...
        $('#discount').text(discount.toFixed());
        var netTotal = total - discount;
        $('#nettotal').text(netTotal.toFixed());
        var money = parseFloat($('#num-pad-input').val()) || 0;
        $('#money').text(money.toFixed());
        var change = money - netTotal;
        $('#change').text(change.toFixed());
    });
</script>
<script>
    var countCategory = 0;
