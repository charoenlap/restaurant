<?php
	...
	function soap($arr = array()){
		...
		$params = $arr['params'];
		$func = trim($arr['func']);
		$response = $client->__soapCall($func, array($params));
		return $response;
	}
	function province(){
		$province = array(
	...
	function uploadMunti($var=array(),$path,$img_profile_name=""){
		...
		// $var = $_FILES[$var];
		$result = false;
		if(move_uploaded_file($var,$path.$img_profile_name)){
			$result = true;
		}
		return $result;
	}
	...
	function apiXML($path,$type = 'get',$params = array(), $output = NULL){
		...
		$exec = curl_exec($cURL);
		$result_xml = simplexml_load_string($exec, "SimpleXMLElement", LIBXML_NOCDATA);
		// var_dump($result_xml);
		// exit();	
